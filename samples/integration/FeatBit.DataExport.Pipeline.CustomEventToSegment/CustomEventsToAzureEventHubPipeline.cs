...
namespace FeatBit.DataExport.Pipelines.AzureEventHub
{
    public class CustomEventsToAzureEventHubPipeline
    {
        public async static Task ExportAsync(ParamOptions parameters, AzureEventHubWriter azEvtHubService)
        {
            ...
            while (true)
            {
                ...
                if (customEventEvents != null && customEventEvents.Count > 0)
                {
                    ...
                    (bool isSuccess, string timeStamp) = await azEvtHubService.WriteCustomEventEventsBatchAsync(customEventEvents);
